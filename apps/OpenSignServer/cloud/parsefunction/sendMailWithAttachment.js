  ...
  try {
    ...
    if (params.url) {
      try {
        ...
        const downloadUrl = params.url.replace(
          'https://localhost:3001/api',
          'http://localhost:8080'
        );
        // The attachment is immediately loaded into memory for Nodemailer or
        // Mailgun anyway. Download it directly instead of writing a temporary
        // file into the read-only /app working directory.
        ...
        const PdfBuffer = Buffer.from(download.data);
        if (!PdfBuffer.length) {
          throw new Error('Downloaded PDF attachment is empty');
        }
        ...
        const cleanupPaths = [{ path: certificatePath, label: 'certificate' }];
        if (transporterSMTP) {
